<div x-data="{
    ...
    openLightbox(index, images) { 
        if (images && images.length > 0) {
            ...
            this.images = images.map(image => ({...image, path: image.path ? '../' + image.path : '' })); 
        }
    }, 
    nextImage() { 
    ...
    <div x-show="lightbox" class="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center z-50" x-on:click.self="lightbox = false">
        <div class="relative">
            ...
            <img x-bind:src="(images && selectedImageIndex !== null && images[selectedImageIndex]) ? '../' + images[selectedImageIndex].path : ''" class="max-w-full max-h-full" />
