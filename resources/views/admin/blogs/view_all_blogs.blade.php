        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr>
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ $blog->content }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap font-medium flex justify-end">
                    <div class="flex">
                        <x-front-end-btn  linking="{{url('/admin/blogs/update_blog', $blog->id)}}" color="blue" showme="" name="Edit" />
                        <form action="{{ url('delete/'.$blog->id) }}" method="get" onsubmit="return confirm('Are you sure you want to delete this service?');">
                            @csrf
                            @method('DELETE')
