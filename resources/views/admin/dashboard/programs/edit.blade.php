...
@section('content')
    <div class="p-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Program</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="program_type" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                            ...
                            <option value="Internship" {{ old('program_type', $program->program_type) == 'Internship' ? 'selected' : '' }}>Internship</option>
                            <option value="TVET Placement" {{ old('program_type', $program->program_type) == 'TVET Placement' ? 'selected' : '' }}>TVET Placement</option>
                            <option value="Short Program" {{ old('program_type', $program->program_type) == 'Short Program' ? 'selected' : '' }}>Short Program</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="partner_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                            ...
                            <option value="{{ $partner->id }}" {{ old('partner_id', $program->partner_id) == $partner->id ? 'selected' : '' }}>
                                {{ $partner->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="4" required
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">{{ old('description', $program->description) }}</textarea>
                        @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Duration *</label>
                        <input type="text" name="duration" value="{{ old('duration', $program->duration) }}" placeholder="e.g., 12 months" required
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    ...
                    <div>
                        ...
                        <input type="number" name="number_needed" value="{{ old('number_needed', $program->number_needed) }}" min="1" required
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="recruitment_start_date" value="{{ old('recruitment_start_date', $program->recruitment_start_date->format('Y-m-d')) }}" required
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="recruitment_end_date" value="{{ old('recruitment_end_date', $program->recruitment_end_date->format('Y-m-d')) }}" required
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="requirements" rows="4" required
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">{{ old('requirements', $program->requirements) }}</textarea>
                        @error('requirements') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="has_stipend" value="1" {{ old('has_stipend', $program->has_stipend) ? 'checked' : '' }}
                    ...
                    <div>
                        ...
                        <input type="number" name="stipend_amount" value="{{ old('stipend_amount', $program->stipend_amount) }}" step="0.01" min="0"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="stipend_currency" class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                            <option value="ZAR" {{ old('stipend_currency', $program->stipend_currency) == 'ZAR' ? 'selected' : '' }}>ZAR</option>
                            <option value="USD" {{ old('stipend_currency', $program->stipend_currency) == 'USD' ? 'selected' : '' }}>USD</option>
                        </select>
                    </div>

                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="has_accreditation" value="1" {{ old('has_accreditation', $program->has_accreditation) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-kb-600 focus:ring-kb-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="accreditation_details" value="{{ old('accreditation_details', $program->accreditation_details) }}"
                               class="w-full rounded-md border-gray-300 shadow-sm focus:border-kb-500 focus:ring-kb-500">
                    </div>

                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="youth_beneficiaries" value="1" {{ old('youth_beneficiaries', $program->youth_beneficiaries) ? 'checked' : '' }}
                                   class="rounded border-gray-300 text-kb-600 focus:ring-kb-500">
