            ...
            <form action="{{ route('dashboard.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="border border-green-200 rounded-lg p-6 bg-green-50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="flex items-center">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox"
                                       ...
                                       id="mou_signed"
                                       value="1"
                                       {{ old('mou_signed', $partner->mou_signed) ? 'checked' : '' }}
                        ...
                        <div id="mou_date_field" class="{{ old('mou_signed', $partner->mou_signed) ? '' : 'hidden' }}">
                            <label for="mou_date" class="block text-sm font-medium text-gray-700 mb-1">Date Signed</label>
                            <input type="date" name="mou_date" id="mou_date"
                                   value="{{ old('mou_date', $partner->mou_date ? $partner->mou_date->format('Y-m-d') : '') }}"
                        ...
                        <div id="mou_document_field" class="{{ old('mou_signed', $partner->mou_signed) ? '' : 'hidden' }}">
                            <label for="mou_document" class="block text-sm font-medium text-gray-700 mb-1">MOU Document (PDF)</label>
                            @if($partner->mou_document)
                                <div class="mb-2 flex items-center gap-2 text-sm text-green-700">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                    <a href="{{ asset($partner->mou_document) }}" target="_blank" class="underline hover:text-green-900">View current MOU</a>
                                    <span class="text-gray-400">(upload new to replace)</span>
                                </div>
                            @endif
